...
class Sitelink {
    ...
    public function __construct() {
        ...
        // echo $this->CI->config->item('sitelink_corp_code')."ddd";die;
    ...
    // tenantList
    public function find_tenantList_with_location_code() {
        $this->params->sLocationCode = $this->CI->config->item('sitelink_loc_code');
        $this->params->sTenantFirstName = 'FL';
        $this->params->sTenantLastName = '';
        ...
        try {
            ...
            if (count($xml_data_table) > 0) {
                ...
                usort($units, function ($unit1, $unit2) {
                    if ($unit1['UnitTypeID'] == $unit2['UnitTypeID'])
                        return 0;
    ...
    public function CancelReservation($data) {
        unset($this->params->iTenantID);
        // page 178 in api docs
        $this->params->sLocationCode = $data['lo_code']; //$this->CI->config->item('sitelink_loc_code');
        $this->params->WaitingID = $data['waiting_id'];
        $this->params->sTenantID = $data['tanent_id'];
        $this->params->sUnitID = $data['unit_id'];
        $this->params->dNeeded = date("Y-m-d");
        $this->params->sComment = 'No Comment'; //optional
        $this->params->iStatus = 2;
        $this->params->bFollowup = false;
        $this->params->dFollowup = '';
        $this->params->dFollowupLast = '';
        $this->params->iInquiryType = 0;
        $this->params->dcQuotedRate = $data['price'];
        $this->params->dExpires = $data['expiry_date'];
        $this->params->QTRentalTypeID = 3;
        $this->params->QTCancellationTypeID = 6; // Other Reason
        $this->params->sCancellationReason = '';
       
        try {
            $cancel_in = array();
            $data = $this->client->ReservationUpdate($this->params);
            $result = $data->ReservationUpdateResult;
           
            $xml_data = new SimpleXMLElement($result->any);
            $xml_data_rt = $xml_data->NewDataSet->RT;
            $xml_data_table = $xml_data->NewDataSet->Receipts;
            $cancel_in['rt'] = $this->xml2array($xml_data_rt);
            return $cancel_in;
        } catch (Exception $e) {
    ...
    public function get_waiting_id_by_tanent_id($lo_code = '', $tanent_id = '') {
        $this->params->sLocationCode = $lo_code;
        $this->params->iTenantID = $tanent_id;
        //$this->params->TenantID = 90709; 
        try {
            $unit = array();
            $data = $this->client->ReservationBillingInfoByTenantID($this->params); // Page 142 in API Docs
            $result = $data->ReservationBillingInfoByTenantIDResult;
            $xml_data = new SimpleXMLElement($result->any);
            $xml_data_table = $xml_data->NewDataSet->BillingInfo;
            if (count($xml_data_table) > 0) {
                $unit = $this->xml2array($xml_data_table);
                return $unit;
            } else {
